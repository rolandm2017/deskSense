...
def assert_start_times_are_chronological(test_events):
    chronological_sessions = sorted(
        test_events, key=lambda event: event.start_time.dt)
    for i in range(0, len(test_events)):
        assert chronological_sessions[i].start_time == test_events[i].start_time, "Something was not chronological"
...
def get_durations_from_test_data(test_events):
    ...
    for i in range(0, len(test_events)):
        ...
        elapsed_between_sessions: timedelta = test_events[i +
                                                          1].start_time.dt - test_events[i].start_time.dt
