...
#     times_for_window_push = [x.start_time for x in program_events_from_prev_test] + [
#         x.start_time for x in chrome_events_from_prev_test]
...
#     as_dt = [UserLocalTime(x) for x in times_for_window_push if isinstance(
#         x, datetime)]  # this is just appeasing type checking
#     clock_again = MockClock(as_dt)  # type: ignore
...
#         assert program_events_from_prev_test[0].start_time is not None
...
#         assert isinstance(date_of_deduction, datetime)
#         assert chrome_events_from_prev_test[0].start_time is not None
...
#     # total_chrome_time = 9
#     assert production + \
#         leisure == (sum_of_program_times.seconds +
