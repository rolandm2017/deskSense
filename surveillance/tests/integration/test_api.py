...
def mock_mouse_events():
    ...
    for i in range(14):  # 12, 14, 16
        event = MouseMoveDto(i, base_time - timedelta(minutes=i*1), base_time - timedelta(minutes=i*2))
        events.append(event)
    return events
...
def mock_program_events():
    ...
    for i in range(16):  # 12, 14, 16
        event = ProgramDto(i, "Foo", base_time + timedelta(minutes=i), base_time + timedelta(minutes=i*1), i % 2 == 0)
        events.append(event)
    return events
...
async def test_get_keyboard_report(test_client, mock_keyboard_events, mock_surveillance_state):
    ...
    mock_service.get_past_days_events = AsyncMock(return_value=mock_keyboard_events)
...
async def test_get_mouse_report(test_client, mock_mouse_events, mock_surveillance_state):
    ...
    mock_service.get_past_days_events = AsyncMock(return_value=mock_mouse_events)
...
async def test_get_program_report(test_client, mock_program_events, mock_surveillance_state):
    ...
    mock_service.get_past_days_events = AsyncMock(return_value=mock_program_events)
