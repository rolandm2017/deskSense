...
def mock_session_data():
    test_program_session = ProgramSession(
        ...
        start_time=datetime(2025, 2, 1, 1, 0, 4, 0,
...
async def test_find_session(prepare_daos, plain_asm, mock_session_data):
    ...
    try:
        ...
        assert len(
            programs) == 1, "Expected one program record; setup conditions not met"
        assert len(
            domains) == 1, "Expected one domain record; setup conditions not met"
        ...
        whenever = datetime(2025, 3, 2, 12, 59, 0,
                            tzinfo=timezone_for_test_data)
        nonexistent_program = ProgramSession("", "", whenever)
        nonexistent_chrome = ChromeSession(
            domain="", detail="", start_time=whenever)
        ...
        assert program_found.start_time == program_session.start_time.astimezone(
            timezone.utc)
        assert chrome_found.start_time == chrome_session.start_time.astimezone(
            timezone.utc)
    finally:
...
async def test_push_window_ahead(prepare_daos, mock_session_data):
    ...
    initial_write_of_program = convert_back_to_local_tz(
        initial_write_of_program, timezone_for_test_data)
    initial_write_of_chrome = convert_back_to_local_tz(
        initial_write_of_chrome, timezone_for_test_data)
    ...
    assert pro_log.end_time == initial_write_of_program.end_time + \
        timedelta(seconds=10), "Window push failed"
    assert chro_log.end_time == initial_write_of_chrome.end_time + \
        timedelta(seconds=10), "Window push failed!"
