...
class KeyboardCsvWriter:
    ...
    def path_stuff(self):
        ...
        current_file = Path(
            __file__)  # This gets us surveillance/src/productivity_tracker.py
        ...
        if not file_path.exists():
            ...
            with open(file_path, 'w', newline='') as f:
                writer = csv.DictWriter(
                    f, fieldnames=['timestamp', 'event_type', 'x_position', 'y_position'])
                writer.writeheader()
...
class ProgramCsvWriter:
    ...
    def save_session(self, session, special=""):
        ...
        if not file_path.exists():
            with open(file_path, 'w', newline='') as f:
                writer = csv.DictWriter(
                    f, fieldnames=['start_time', 'end_time', 'duration', 'window', 'productive'])
                writer.writeheader()
    ...
    def generate_report(self, date_str=None):
            ...
            for row in reader:
                ...
                if app == 'chrome.exe' and len(parts) > 1:
                    ...
                    for site in self.productive_sites:
                        if site in title:
                            chrome_sites[site] = chrome_sites.get(
                                site, 0) + duration_hours
                    # If not a productive site, group under "other"
                    if not any(site in title for site in self.productive_sites):
                        chrome_sites["other"] = chrome_sites.get(
                            "other", 0) + duration_hours

                # Group by application
                app_name = self.productive_apps.get(app, app)
                app_times[app_name] = app_times.get(
                    app_name, 0) + duration_hours
        ...
        if 'Chrome' in app_times and chrome_sites:
            app_times['Chrome'] = {
                'total': app_times['Chrome'], 'sites': chrome_sites}
