...
class SystemPowerTracker:
    ...
    async def run_shutdown_tasks(self, reason):
        tasks = []

        # Always add the on_power_down task
        tasks.append(self.on_power_down())

        # Add the appropriate status update based on reason
        if reason == "shutdown":
            tasks.append(self.system_status_dao.create_status(
                SystemStatusType.SHUTDOWN, datetime.now()))
        elif reason == "restart program":
            tasks.append(self.system_status_dao.create_status(
    ...
    def _system_shutdown_handler(self, signum, frame, reason="shutdown"):
        ...
        future = asyncio.run_coroutine_threadsafe(
            self.run_shutdown_tasks(reason), self.asyncio_loop)
        ...
        if sleeping:
            ...
            future = asyncio.run_coroutine_threadsafe(
                self.system_status_dao.create_status(
                    SystemStatusType.SLEEP, datetime.now()), self.asyncio_loop)
            try:
                future.result(timeout=2)  # Wait up to 2 seconds
            except Exception as e:
                print(e)
                print(f"Failed to log sleep event: {e}")

            # self.system_status_dao.create_status(
            #     SystemStatusType.SLEEP, datetime.now())
            self._system_shutdown_handler('sleep', None, "Sleep")
        else:
            print(f"System waking up at {datetime.now()}")
            # NOTE: Does not need to block, it runs on wake!
            self.system_status_dao.create_status(
