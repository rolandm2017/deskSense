...
class SessionIntegrityDao:
    ...
    def audit_sessions(self, latest_shutdown_time: datetime, startup_time: datetime):
        ...
        if a == 0 and b == 0 and c == 0 and d == 0:
            self.logger.log_white_multiple("[info] Orphans and phantoms: 0, 0, 0, 0")
        else:
            self.logger.log_red_multiple("[debug] Orphans and phantoms found:", a, b, c, d)
            self.logger.log_red(
                "[debug] Program was offline between "
                + str(latest_shutdown_time)
                + " and "
                + str(startup_time)
            )
    ...
    def audit_first_startup(self, startup_time: datetime):
        ...
        if a == 0 and b == 0 and c == 0 and d == 0:
            ...
            self.logger.log_red_multiple("Orphans and phantoms found:", a, b, c, d)
            ...
            print_and_log(program_phantoms, the_beginning_of_time, startup_time)
