...
def test_get_user_timezone():
    tz_for_user = timezone_service.get_tz_for_user("whatever for now")
...
def test_convert_tab_change_timezone():
    ...
    some_tab_change_event = UtcDtTabChange(tabTitle=tab_title, url=url, startTime=start_time)
    tz_for_user = timezone_service.get_tz_for_user("whatever for now")
...
def test_real_scenario():
    ...
    event = UtcDtTabChange(tabTitle="foo", url="bar", startTime=march_16_at_1_am)
    tz_for_user = timezone_service.get_tz_for_user(9000)
    updated_tab_change_event = timezone_service.convert_tab_change_timezone(
        event, tz_for_user
    )
