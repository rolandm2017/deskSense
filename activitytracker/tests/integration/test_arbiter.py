...
def test_activity_arbiter(activity_arbiter_and_setup):
    ...
    def assert_basic_expectations_met():
        ...
        assert all(
            isinstance(log, ProgramSession) for log in program_events
        ), "A program event wasn't a program session"
        assert all(
            isinstance(log, ChromeSession) for log in chrome_events
        ), "A Chrome event wasn't a Chrome session"

        # Because .duration is set in the ASM before on_state_changed:
        assert all(
            isinstance(obj.duration, timedelta)
    ...
    assert (
        len(mock_activity_recorder.add_ten_sec_to_end_time.call_args_list)
    ...
    def assert_keep_alive_worked_as_intended():
        ...
        for i in range(0, len(copied_test_data)):
            ...
            for j in range(
                end_of_prev_calls, corresponding_num_of_pushes + end_of_prev_calls
            ):
                end_of_prev_calls += 1
                session_from_spy = (
            ...
            assert (
                target_session.start_time.dt == add_partial_window_session_arg.start_time.dt
            )
    ...
    assert len(program_events) == len(
        program_sessions_in_test
    ), "A program session didn't make it through"
    # Total number of recorded Chrome DAO Entries was as expected
    remaining_open_session_offset = 1
    assert (
        len(chrome_events) == len(chrome_sessions_in_test) - remaining_open_session_offset
    ), "A Chrome session didn't make it through"
    ...
    assert all(
        log.end_time is not None for log in events_from_on_state_changed_handler
    ), "An end time wasn't set"
    # FIXME: should also check that the duration is 10 sec despite the 0.1 sec pulse
    assert all(
        log.duration is not None for log in events_from_on_state_changed_handler
    ...
    for i in range(0, len(chronological)):
        ...
        assert (
            current.duration == duration_from_start_end_times
        ), "A duration did not calculate correctly"
    ...
    def assert_time_matches_in_on_state_changed():
        ...
        assert (
            elapsed_time_in_test == minutes_between_start_and_2nd_to_last
        ), f"The elapsed time was not as expected, perhaps due to interval pulse"
    ...
    def assert_expected_values_match_ledger():
        ...
        for i in range(0, len(copied_test_data)):
            ...
            if i == len(copied_test_data) - 1:
                ...
                assert copied_test_data[i].ledger.get_total() == 0
                break
    ...
    # Because even 0 is a partial
    total_expected_partials = len(durations_between_events_from_setup)
    ...
    for i in range(0, len(copied_test_data)):
        ...
        print(zip)
        assert (
            ledger_amt == expected_duration
        ), f"Failed on index {i} of {len(copied_test_data) - 1}"
        assert (
            keep_alive_tally == expected_duration
        ), f"Failed on index {i} of {len(copied_test_data) - 1}"

        zips.append(zip)

    def assert_time_matches_in_keep_alive():
        num_of_window_pushes = len(
            mock_activity_recorder.add_ten_sec_to_end_time.call_args_list
        )
